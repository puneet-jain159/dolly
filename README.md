...
- Open the `train_dolly_multi_gpu` notebook in the Repo (which is the `train_dolly_multi_gpu.py` file in the Github `dolly` repo), attach to your GPU cluster, and run all cells.  When training finishes, the notebook will save the model under `local_dir =  f"/dbfs/{username}/dolly_train/job/`.
